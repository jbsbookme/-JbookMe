...
export default function AdminServiciosMujeresPage() {
  ...
  const [_genderFilter, _setGenderFilter] = useState<string>('FEMALE'); // FIXED: Only show women's services
  ...
  const [newServiceForm, setNewServiceForm] = useState({
    ...
    image: '',
    barberId: '',
    gender: 'FEMALE', // FIXED: Always default to FEMALE
  });
  ...
  const [editServiceForm, setEditServiceForm] = useState({
    ...
    image: '',
    barberId: '',
    gender: 'FEMALE', // FIXED: Por defecto siempre FEMALE
    isActive: true,
  });
  ...
  const fetchServices = async () => {
    try {
      ...
      const response = await fetch('/api/services?gender=FEMALE');
      if (response.ok) {
  ...
  const fetchBarbers = async () => {
    try {
      // FIXED: Solo cargar estilistas HOMBRES
  ...
  // FIXED: Solo mostramos servicios de FEMALE, no necesitamos filtrar más
  const filteredServices = services;

  // Count services by gender (simplificado - solo FEMALE)
  const _serviceCounts = {
    all: services.length,
    male: services.length, // Todos son FEMALE
    female: 0,
    unisex: 0,
  };
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold text-white mb-2">
              Services for <span className="text-[#00f0ff]">Women</span>
            </h1>
            <p className="text-gray-400">Manage services for stylists (women)</p>
          </div>
          <Button
              ...
              >
                <CardContent className="p-6">
                  ...
                  <div className="flex items-start justify-between mb-2">
                    ...
                    >
                      {service.gender === 'FEMALE' ? '👩 Women' : service.gender === 'MALE' ? '👨 Men' : '👥 Unisex'}
                    </span>
                  </div>
                  {service.description && (
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="bg-[#1a1a1a] border-gray-800 text-white max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="barber" className="text-gray-300">Specific stylist (optional)</Label>
              <select
                id="barber"
              ...
              >
                <option value="">General (all stylists)</option>
                {barbers.map((barber) => (
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="bg-[#1a1a1a] border-gray-800 text-white max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="edit-barber" className="text-gray-300">Specific stylist (optional)</Label>
              <select
                id="edit-barber"
